...
Most of the times the batch is compressed. As of now there are only 4 algorithms for compression it understands (`gzip`, `snappy`, `lz4`, `zstd`). The batch has all needed metadata over the compressed bytes as well as an integrity checksum (`CRC-32C`).

Here's how batch looks like in bytes:
...
You might have noticed that Kafka either has numeric values of fixed size bytes, or byte arrays to represent dynamic data, like keys and values. Kafka explicitly opts for byte representation instead of Strings, JSON, etc., because it works with any format you build on top. Part of the complexity of using Kafka is that you have to do all the work above, like chosing a consistent representation, perform serialization, etc.
...
Finally we need a logic way to combine the log segments into a single location. Kafka creates folders based on _topic_ plus _partition_. I won't explain how the partitions work, because these created with the consumer in mind. I'll go over the mechanics of that in a future blog post. 
